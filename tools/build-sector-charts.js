...
// Phase 205 / Workstreams A+B — isolated sector institutional chart pipeline.
// Reuses the asset chart engine (buildChart/renderSvg) + fetch helpers, writing a
// SEPARATE manifest (data/visual/sector-charts.json) + SVG dir so the asset
...
//
// Output: data/visual/sector-charts.json + data/visual/sector-charts/<id>-{en,ar}.svg
// Usage:  node tools/build-sector-charts.js [--fetch] [--write]

const fs = require('fs');
const path = require('path');
const { SECTORS, chartId } = require('./sector-registry');
const eng = require('./build-institutional-charts');

const ROOT = path.resolve(__dirname, '..');
const OUT = path.join(ROOT, 'data', 'visual', 'sector-charts.json');
const OUT_DIR = path.join(ROOT, 'data', 'visual', 'sector-charts');
...
function specFor(sector) {
  return {
    id: chartId(sector.symbol), symbol: sector.symbol, visual_type: 'sector_structure',
    title_en: `${sector.name_en} sector observed structure`,
    title_ar: `البنية المرصودة لقطاع ${sector.name_ar}`,
    allowed_surfaces: ['sectors'], related_topics: [sector.slug, sector.group],
  };
}
...
async function build() {
  ...
  for (const sector of SECTORS) {
    ...
    built.chart.files = { en: `data/visual/sector-charts/${id}-en.svg`, ar: `data/visual/sector-charts/${id}-ar.svg` };
    built.chart.sector_slug = sector.slug;
    rendered.push(built);
  }
  return {
    ...
    provider_health: { mode: FETCH ? 'fetch' : 'offline', checked_at: new Date().toISOString(), assets_total: SECTORS.length, assets_available: rendered.length, assets_unavailable: unavailable.length, rate_limited: diagnostics.some((d) => d.rate_limited) },
    diagnostics,
...
async function main() {
  ...
  console.log(`[sector-charts] status=${result.status} charts=${result.charts.length} unavailable=${result.unavailable.length}`);
  if (!WRITE) return;
  ...
  console.log(`[sector-charts] wrote manifest + ${result.charts.length * 2} SVG(s)`);
}

if (require.main === module) { main().catch((e) => { console.error(`[sector-charts] ${e.stack || e.message}`); process.exit(1); }); }
