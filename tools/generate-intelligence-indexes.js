...
const SURFACES = {
  // NOTE: `markets` was removed here. /markets/ is now generated by
  // tools/generate-hub-pages.js as a premium Markets Hub. The asset
  // listing it used to host now lives at /markets/assets/ below so
  // the Markets Hub "Assets" card has a real destination, and per-
  // asset detail pages keep their /markets/<slug>/ paths via the
  // asset-pages generator.
  marketsAssets: {
    base: 'markets/assets', title_en: 'Institutional Asset Intelligence', title_ar: 'استخبارات الأصول المؤسسية',
    ...
    items: () => ASSETS.map((a) => ({ slug: a.slug, name_en: a.symbol, name_ar: a.symbol, role_en: a.role_en, role_ar: a.role_ar, hrefOverride: `/markets/${a.slug}/`, hrefOverrideAr: `/ar/markets/${a.slug}/` })),
  },
  sectors: {
...
function buildMain(ar, surf) {
  ...
  const cards = items.map((i) => {
    const href = ar
      ? (i.hrefOverrideAr || `/ar/${surf.base}/${i.slug}/`)
      : (i.hrefOverride || `/${surf.base}/${i.slug}/`);
    return `          <article class="market-card"><span class="market-card-kicker">${esc(ar ? i.name_ar : i.name_en)}</span><h3><a href="${esc(href)}">${esc(ar ? i.role_ar : i.role_en)}</a></h3></article>`;
  }).join('\n');
