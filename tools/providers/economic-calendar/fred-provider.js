...
async function fetchCalendar(context) {
  ...
  const url = `${ENDPOINT}?api_key=${encodeURIComponent(apiKey)}&file_type=json&include_release_dates_with_no_data=true&limit=1000&order_by=release_date&sort_order=desc`;
  ...
  if (!relevant.length) throw providerError('no_matching_release_dates');
