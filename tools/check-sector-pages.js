...
// Phase 205 / Workstream I — check:sector-pages.
// Integrity gate for /sectors/<slug>/ + /ar/sectors/<slug>/. HARD-FAILS on a
...
// sector-charts manifest) / non-responsive / missing caption, or a chartless page
...
const REQUIRED = ['sector-intelligence', 'sector-ranking-position', 'sector-chart', 'sector-rotation', 'sector-macro', 'sector-history-context', 'sector-narrative-context', 'sector-disclaimer'];
...
function validateFigure(fig, lang, hashes, label, out) {
  ...
  else if (hashes.size && !hashes.has(sh)) out.push(`${label}: chart series_hash not in sector manifest (placeholder/fabricated)`);
...
function validatePage(rel, lang, slug, hashes, out) {
  ...
  const canonical = `https://www.tradealphaai.com/${lang === 'ar' ? 'ar/' : ''}sectors/${slug}/`;
...
function run() {
  ...
  for (const sector of SECTORS) {
    const enRel = `sectors/${sector.slug}/index.html`; const arRel = `ar/sectors/${sector.slug}/index.html`;
    const en = fs.existsSync(path.join(ROOT, enRel)); const ar = fs.existsSync(path.join(ROOT, arRel));
    if (!en && !ar) continue; any = true;
    if (!en || !ar) { out.push(`${sector.symbol}: missing ${en ? 'AR' : 'EN'} page (parity break)`); continue; }
    const enIds = validatePage(enRel, 'en', sector.slug, hashes, out);
    const arIds = validatePage(arRel, 'ar', sector.slug, hashes, out);
    if (enIds.join('|') !== arIds.join('|')) out.push(`${sector.symbol}: EN/AR section parity break`);
    scanned += 2;
  }
...
if (require.main === module && process.argv.includes('--self-test')) {
  ...
  const good = '<figure class="institutional-chart" data-symbol="XLK" data-chart-type="sector_structure" data-series-hash="realhash" data-as-of="2026-06-16"><div class="ic-svg"><svg viewBox="0 0 1400 900" direction="rtl"><polyline points="1,2 3,4"/></svg></div><figcaption class="ic-caption"><span class="ic-attrib">Source: Yahoo · As of 2026-06-16</span></figcaption></figure>';
  const cases = [
  ...
  console.log(`[sector-pages] self-test: ${ok}/${cases.length + 1} passed`);
  process.exit(ok === cases.length + 1 ? 0 : 1);
}

if (require.main === module) {
  const { failures, scanned, skipped } = run();
  if (skipped) { console.log('[sector-pages] no sector pages built yet (non-fatal).'); process.exit(0); }
  if (failures.length) { failures.forEach((m) => console.error(`[sector-pages] FAIL: ${m}`)); process.exit(1); }
  console.log(`[sector-pages] check:sector-pages passed (${scanned} page(s); EN/AR parity, RTL, required sections, real charts only, honest unavailable, no retail/advice).`);
}
