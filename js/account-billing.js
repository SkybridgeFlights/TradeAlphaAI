...
section.addEventListener('click',async e=>{const tier=e.target.getAttribute('data-tier');const portal=e.target.hasAttribute('data-portal');if(!tier&&!portal)return;try{set(ar?'جارٍ فتح Stripe…':'Opening Stripe…',true);const j=await call(portal?'/api/account/billing-portal':'/api/account/billing-checkout',portal?{}:{tier});if(j.url)location.assign(j.url);else throw new Error('missing_redirect');}catch(err){set(err.message,false);}});
load();
})();
